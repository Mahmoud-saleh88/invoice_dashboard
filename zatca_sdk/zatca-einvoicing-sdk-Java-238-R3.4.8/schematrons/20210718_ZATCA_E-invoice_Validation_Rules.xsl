            ...
            <schxslt:rule pattern="d7e416">
               ...
               <xsl:if test="string-length(./cbc:Amount) &gt; 0 and string-length(./cbc:BaseAmount) &gt; 0 and not ( format-number(./cbc:Amount,'#.00') =  format-number(( floor((((./cbc:BaseAmount * ./cbc:MultiplierFactorNumeric) div 100) * 100 + 0.5)) div 100), '#.00'))">
                  <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                     ...
                     <xsl:attribute name="test">
                     	<!--xs:decimal(./cbc:Amount) = (xs:decimal(./cbc:BaseAmount) * xs:decimal(./cbc:MultiplierFactorNumeric) div 100) format-number((cbc:TaxAmount),'#.00')--> 
                     format-number(../cbc:Amount,'#.00') =  format-number(((../cbc:BaseAmount * ../cbc:MultiplierFactorNumeric) div 100) ,'#.00')
                     	</xsl:attribute>
                     <svrl:text>[BR-KSA-EN16931-03]-Allowance amount (BT-92, BT-136) must equal base amount (BT-93, BT-137) * percentage (BT-94, BT-138) / 100 if base amount and percentage exists.And rounded to 2 decimal places.</svrl:text>
