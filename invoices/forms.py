...
class SalesInvoiceItemForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            # ✅ DecimalField — يقبل أرقاماً كسرية مثل 2.5 أو 1.750
            'quantity': forms.NumberInput(attrs={
                'class': f'{_cls} item-quantity',
                'step':  '0.001',   # 3 خانات عشرية
                'min':   '0.001',
                'value': '1',
            }),
...
class SalesInvoiceForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'invoice_number': forms.TextInput(attrs={'class': _cls,'readonly': 'readonly','style': 'background-color: #e9ecef;'}),
            'date': forms.DateInput(attrs={'type': 'date', 'class': _cls}),
            'supply_date': forms.DateInput(attrs={'type': 'date','class': _cls,'placeholder': 'تاريخ التوريد (مطلوب للفواتير الضريبية)',}),
            ...
            'created_by': forms.TextInput(attrs={'class': _cls,'readonly': 'readonly','style': 'background-color: #e9ecef;'}),
            'tax_rate': forms.NumberInput(attrs={'class': _cls,'step': '0.01','min': '0','id': 'id_tax_rate'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:
            ...
            self.fields['supply_date'].initial = today
...
class ClientForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'street_name': forms.TextInput(attrs={'class': _cls,'placeholder': 'اسم الشارع — مطلوب لزاتكا (BR-KSA-63)',}),
            'building_number': forms.TextInput(attrs={'class': _cls,'placeholder': '4 أرقام — مثل: 1234','maxlength': '4',}),
            'district': forms.TextInput(attrs={'class': _cls,'placeholder': 'الحي — مطلوب لزاتكا',}),
            'city': forms.TextInput(attrs={'class': _cls, 'placeholder': 'المدينة'}),
            'postal_code': forms.TextInput(attrs={'class': _cls,'placeholder': '5 أرقام — مثل: 12345','maxlength': '5',}),
            'vat_number': forms.TextInput(attrs={'class': _cls,'placeholder': '15 رقماً يبدأ وينتهي بـ 3',}),
...
class JournalEntryForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'number': forms.TextInput(attrs={'class': _cls,'readonly': 'readonly','style': 'background-color: #e9ecef;','placeholder': 'سيتم توليده تلقائياً'}),
            'date': forms.DateInput(attrs={'type': 'date','class': _cls,}),
            'entry_type': forms.Select(attrs={'class': _cls,}),
            'description': forms.Textarea(attrs={'class': _cls,'rows': 3,'placeholder': 'أدخل وصف القيد المحاسبي',}),
            'reference': forms.TextInput(attrs={'class': _cls,'placeholder': 'رقم المستند / المرجع (اختياري)',}),
            'notes': forms.Textarea(attrs={'class': _cls,'rows': 2,'placeholder': 'ملاحظات إضافية (اختياري)',}),
        }
        labels = {
            'number': 'رقم القيد',
            'date': 'التاريخ',
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:
            self.fields['number'].initial = self.generate_entry_number()
    ...
    def clean_number(self):
        ...
        if not number:
            number = self.generate_entry_number()
    ...
    def clean(self):
        ...
        if not cleaned_data.get('number'):
            cleaned_data['number'] = self.generate_entry_number()
...
class JournalEntryLineForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'account': forms.Select(attrs={'class': f'{_cls} line-account','placeholder': 'الحساب',}),
            'description': forms.TextInput(attrs={'class': _cls,'placeholder': 'البيان',}),
            'debit_amount': forms.NumberInput(attrs={'class': f'{_cls} line-debit debit-input','step': '0.01','min': '0',}),
            'credit_amount': forms.NumberInput(attrs={'class': f'{_cls} line-credit credit-input','step': '0.01','min': '0',}),
            'cost_center': forms.Select(attrs={'class': _cls}),
        }
 
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['account'].queryset = Account.objects.filter(account_type='detail', is_active=True).order_by('code')
...
class BaseJournalEntryLineFormSet(BaseInlineFormSet):
    ...
    def should_delete_form(self, form):
        cd = getattr(form, 'cleaned_data', {})
        if cd.get('_is_empty'):
            return True
        return super().should_delete_form(form)
    ...
    def clean(self):
        ...
        for form in self.forms:
            ...
            dr            = form.cleaned_data.get('debit_amount')  or Decimal('0')
            cr            = form.cleaned_data.get('credit_amount') or Decimal('0')

            # ✅ لا نشترط has_changed() — نقبل أي صف له حساب أو مبلغ
